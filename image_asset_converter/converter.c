...
typedef enum { RGB, RGBA, NormalMap } OutputFormat;
...
static int parse_import_file(void)
{
	...
	const char* keys[] = { "FORMAT", "CREATE-MIPMAP", "NO-BC1-OR-3", "NO-BC5", "NO-BC7" };
...
static void get_mip_map_requirements(unsigned int width, unsigned int height, unsigned int bytes_per_4x4_block,
	bool use_mip_maps, unsigned int* levels, unsigned int* bytes_total)
{
	if (!use_mip_maps) {
		*levels = 1;
...
static int alloc_memory_and_convert_raw(unsigned int image_width, unsigned int image_height, void** raw_data,
	void** uncompressed_data, unsigned int* uncompressed_data_size, void* lossy_compressed_data[2],
	unsigned int lossy_compressed_data_size[2], unsigned int* mip_levels)
{
	if (output_format == NormalMap) {
		get_mip_map_requirements(image_width, image_height, 32, generate_mipmap, mip_levels, uncompressed_data_size);
		*lossy_compressed_data_size = use_bc5 ? *uncompressed_data_size / 2 : 0;
	} else {
		get_mip_map_requirements(image_width, image_height, 64, generate_mipmap, mip_levels, uncompressed_data_size);
...
static void mip_downscale(uint8_t* mip_dst, uint8_t* mip_src, unsigned int mip_width, unsigned int x, unsigned int y,
...
static void generate_uncompressed_mipmap(unsigned int image_width, unsigned int image_height, void* uncompressed_data)
{
...
static void generate_lossy_compressed_mip_non_bc7(
	void* uncompressed_data, void* lossy_compressed_data, unsigned int width, unsigned int height)
{
	unsigned int bytes_per_pixel = output_format == NormalMap ? 2 : 4;
	...
	for (unsigned int y = 0; y < height; y += 4) {
		for (unsigned int x = 0; x < width; x += 4) {
			...
			memcpy(
				&block_data[bytes_per_pixel * 4], &src[((y + 1) * width + x) * bytes_per_pixel], bytes_per_pixel * 4);
			memcpy(&block_data[bytes_per_pixel * 4 * 2], &src[((y + 2) * width + x) * bytes_per_pixel],
				bytes_per_pixel * 4);
			memcpy(&block_data[bytes_per_pixel * 4 * 3], &src[((y + 3) * width + x) * bytes_per_pixel],
				bytes_per_pixel * 4);
...
static int generate_lossy_compressed(
	void* uncompressed_data_, void* lossy_compressed_data_[2], unsigned int width_, unsigned int height_)
{
	...
	for (unsigned int i = 0; i < 2; i++) {
		...
		while (width >= 4 && height >= 4) {
			if (is_bc7) {
				...
				bc7e_compress_blocks_avx(
					blocks, (uint64_t*)lossy_compressed_data[i], (const uint32_t*)bc7_block_data, &bc7_params);
#endif
			} else {
...
static int do_zstd_compress(
	FILE* f, void* data_in, unsigned int data_in_size, bool* zstd, unsigned int* zstd_compressed_length)
{
	size_t zstd_max_length = ZSTD_compressBound(data_in_size);
	void* zstd_compressed_data = malloc(zstd_max_length);
	if (!zstd_compressed_data)
		return 1;

	size_t output_bytes = ZSTD_compress(zstd_compressed_data, zstd_max_length, data_in, data_in_size, 3);
...
static int save_files(void* uncompressed_data, unsigned int uncompressed_data_length, void* lossy_compressed_data[2],
	unsigned int lossy_compressed_data_size_bytes[2], unsigned int image_width, unsigned int image_height)
{
	...
	if (do_zstd_compress(data_file, uncompressed_data, uncompressed_data_length, &uncompressed_uses_zstd,
			&uncompressed_data_length_zstd_length))
		return 1;
	...
	for (unsigned int i = 0; i < 2; i++) {
		if (lossy_compressed_data_size_bytes[i]) {
			if (do_zstd_compress(data_file, lossy_compressed_data[i], lossy_compressed_data_size_bytes[i],
					&lossy_compressed_uses_zstd[i], &lossy_compressed_data_length_zstd_length[i]))
				return 1;
			subr_count++;
		} else
			break;
	...
	fprintf(asset_file, "#ASSET %s\nTYPE IMAGE\nWIDTH %d\nHEIGHT %d\nIMAGE-FORMAT %s\n", image_file_path, image_width,
		image_height, output_format == NormalMap ? "RG" : (output_format == RGB ? "RGB" : "RGBA"));
...
int main(int argc, const char** argv)
{
	...
	if (load_image_file(&raw_data, (int*)&image_width, (int*)&image_height))
		return 1;
	...
	if (alloc_memory_and_convert_raw(image_width, image_height, &raw_data, &uncompressed_data, &uncompressed_data_size,
			lossy_compressed_data, lossy_compressed_data_size_bytes, &mip_levels))
		return 1;
	...
	if (generate_lossy_compressed(uncompressed_data, lossy_compressed_data, image_width, image_height))
		return 1;

	if (save_files(uncompressed_data, uncompressed_data_size, lossy_compressed_data, lossy_compressed_data_size_bytes,
