...
PIGEON_ERR_RET pigeon_wgi_create_framebuffer_images(FramebufferImageObjects* objects, PigeonWGIImageFormat format,
	unsigned int width, unsigned int height, bool to_be_transfer_src, bool to_be_transfer_dst, bool shadow)
{
	...
	if (VULKAN) {
		...
		if (pigeon_vulkan_allocate_memory_dedicated(
				&objects->memory, memory_requirements, memory_preferences, &objects->image, NULL))
			return 1;
		if (pigeon_vulkan_image_bind_memory_dedicated(&objects->image, &objects->memory))
			return 1;
		...
		pigeon_opengl_set_texture_sampler(
			&objects->gltex2d, !pigeon_wgi_image_format_is_depth(format), false, shadow, true, false);
	}
...
PIGEON_ERR_RET pigeon_wgi_create_framebuffers(void)
{
	...
	if (pigeon_wgi_create_framebuffer_images(&singleton_data.depth_image, PIGEON_WGI_IMAGE_FORMAT_DEPTH_F32,
			sc_info.width, sc_info.height, false, false, false))
		return 1;

	if (VULKAN) {
		ASSERT_R1(!pigeon_vulkan_create_framebuffer(
			&singleton_data.depth_framebuffer, &singleton_data.depth_image.image_view, NULL, &singleton_data.rp_depth));
	} else {
		ASSERT_R1(!pigeon_opengl_create_framebuffer(
			&singleton_data.gl.depth_framebuffer, &singleton_data.depth_image.gltex2d, NULL));
	}
	...
	if (singleton_data.full_render_cfg.ssao) {
		...
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.ssao_images[1], PIGEON_WGI_IMAGE_FORMAT_R_U8_LINEAR,
				sc_info.width / 4, sc_info.height / 4, false, false, false))
			return 1;
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.ssao_images[2], PIGEON_WGI_IMAGE_FORMAT_R_U8_LINEAR,
				sc_info.width / 4, sc_info.height / 4, false, false, false))
			return 1;

		if (VULKAN) {
			for (unsigned int i = 0; i < 3; i++) {
				ASSERT_R1(!pigeon_vulkan_create_framebuffer(&singleton_data.ssao_framebuffers[i], NULL,
					&singleton_data.ssao_images[i].image_view, &singleton_data.rp_ssao));
			}
		} else {
			for (unsigned int i = 0; i < 3; i++) {
				ASSERT_R1(!pigeon_opengl_create_framebuffer(
					&singleton_data.gl.ssao_framebuffers[i], NULL, &singleton_data.ssao_images[i].gltex2d));
			}
		}
	}
	...
	if (pigeon_wgi_create_framebuffer_images(
			&singleton_data.render_image, hdr_format, sc_info.width, sc_info.height, false, false, false))
		return 1;
	...
	if (singleton_data.full_render_cfg.bloom) {
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.bloom_images[0][0], hdr_format, sc_info.width / 2,
				sc_info.height / 2, false, false, false))
			return 1;
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.bloom_images[0][1], hdr_format, sc_info.width / 2,
				sc_info.height / 2, false, false, false))
			return 1;
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.bloom_images[1][0], hdr_format, sc_info.width / 4,
				sc_info.height / 4, false, false, false))
			return 1;
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.bloom_images[1][1], hdr_format, sc_info.width / 4,
				sc_info.height / 4, false, false, false))
			return 1;
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.bloom_images[2][0], hdr_format, sc_info.width / 8,
				sc_info.height / 8, false, false, false))
			return 1;
		if (pigeon_wgi_create_framebuffer_images(&singleton_data.bloom_images[2][1], hdr_format, sc_info.width / 8,
				sc_info.height / 8, false, false, false))
			return 1;
		...
		if (VULKAN) {
			for (unsigned int i = 0; i < 3; i++) {
				for (unsigned int j = 0; j < 2; j++) {
					ASSERT_R1(!pigeon_vulkan_create_framebuffer(&singleton_data.bloom_framebuffers[i][j], NULL,
						&singleton_data.bloom_images[i][j].image_view, &singleton_data.rp_bloom_blur));
				}
			}
		} else {
			for (unsigned int i = 0; i < 3; i++) {
				for (unsigned int j = 0; j < 2; j++) {
					ASSERT_R1(!pigeon_opengl_create_framebuffer(
						&singleton_data.gl.bloom_framebuffers[i][j], NULL, &singleton_data.bloom_images[i][j].gltex2d));
				}
			}
		}
	}
...
{
	if (VULKAN) {
		if (objects->image_view.vk_image_view)
			pigeon_vulkan_destroy_image_view(&objects->image_view);
		if (objects->image.vk_image)
			pigeon_vulkan_destroy_image(&objects->image);
		if (objects->memory.vk_device_memory)
			pigeon_vulkan_free_memory(&objects->memory);
	} else if (objects->gltex2d.id)
		pigeon_opengl_destroy_texture(&objects->gltex2d);
}
...
void pigeon_wgi_destroy_framebuffers(void)
{
	if (VULKAN) {
		...
		for (unsigned int i = 0; i < 3; i++) {
			pigeon_vulkan_destroy_framebuffer(&singleton_data.ssao_framebuffers[i]);
		}
		...
		for (unsigned int i = 0; i < 3; i++) {
			pigeon_opengl_destroy_framebuffer(&singleton_data.gl.ssao_framebuffers[i]);
		}
	...
	for (unsigned int i = 0; i < 3; i++) {
		destroy_framebuffer_images(&singleton_data.ssao_images[i]);
	}
