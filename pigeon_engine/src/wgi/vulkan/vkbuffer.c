...
PIGEON_ERR_RET pigeon_vulkan_buffer_bind_memory(
	PigeonVulkanBuffer* buffer, PigeonVulkanMemoryAllocation* memory, uint64_t offset)
{
	...
	ASSERT_LOG_R1(vkBindBufferMemory(vkdev, buffer->vk_buffer, memory->vk_device_memory, offset) == VK_SUCCESS,
		"vkBindBufferMemory error");
	return 0;
}

PIGEON_ERR_RET pigeon_vulkan_buffer_bind_memory_dedicated(
	PigeonVulkanBuffer* buffer, PigeonVulkanMemoryAllocation* memory)
{
	ASSERT_R1(buffer && buffer->vk_buffer && memory && memory->vk_device_memory);
	ASSERT_LOG_R1(vkBindBufferMemory(vkdev, buffer->vk_buffer, memory->vk_device_memory, 0) == VK_SUCCESS,
		"vkBindBufferMemory error");
	return 0;
}
...
static PIGEON_ERR_RET create_device_local(
	PigeonVulkanBuffer* buffer, PigeonVulkanMemoryAllocation* memory, uint64_t size, PigeonVulkanBufferUsages usages)
{
	...
	uint64_t size, PigeonVulkanBufferUsages usages, bool maybe_device_local)
{
	...
	PigeonVulkanMemoryTypePreferences preferences = { 0 };
	preferences.device_local
		= maybe_device_local ? PIGEON_VULKAN_MEMORY_TYPE_PREFERRED : PIGEON_VULKAN_MEMORY_TYPE_MUST_NOT;
...
PIGEON_ERR_RET pigeon_vulkan_create_staged_buffer(
	PigeonVulkanStagedBuffer* buffer, uint64_t size, PigeonVulkanBufferUsages usages)
{
...
void pigeon_vulkan_staged_buffer_transfer(
	PigeonVulkanStagedBuffer* buffer, PigeonVulkanCommandPool* pool, unsigned int command_pool_buffer_index)
{
	assert(buffer && buffer->buffer.vk_buffer && buffer->staging_buffer.vk_buffer);
	assert(!buffer->buffer_is_host_visible);

	pigeon_vulkan_buffer_transfer(
		pool, command_pool_buffer_index, &buffer->buffer, 0, &buffer->staging_buffer, 0, buffer->buffer.size);
}
	...
	PigeonVulkanBuffer* buffer, PigeonVulkanMemoryAllocation* memory, uint64_t size, void** mapping)
{
	...
	if (pigeon_vulkan_buffer_bind_memory_dedicated(buffer, memory) || pigeon_vulkan_map_memory(memory, mapping)) {
		pigeon_vulkan_destroy_buffer(buffer);
		pigeon_vulkan_free_memory(memory);
		return 1;
	}
