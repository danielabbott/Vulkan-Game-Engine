	...
	VkAttachmentReference* depth_attachment_ref)
{
	...
	if (config.colour_image != PIGEON_WGI_IMAGE_FORMAT_NONE) {
		...
		attachments[0].loadOp
			= config.clear_colour_image ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		attachments[0].storeOp = VK_ATTACHMENT_STORE_OP_STORE;
		attachments[0].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		attachments[0].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		attachments[0].initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		attachments[0].finalLayout = config.colour_image_is_swapchain
	...
	if (config.colour_image2 != PIGEON_WGI_IMAGE_FORMAT_NONE) {
		...
		attachments[1].loadOp
			= config.clear_colour_image2 ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
...
{
	assert(
		subpass && subpass_dependencies && subpass_dependency_count && colour_attachment_refs && depth_attachment_ref);
	...
	subpass->pDepthStencilAttachment
		= config.depth_mode != PIGEON_VULKAN_RENDER_PASS_DEPTH_NONE ? depth_attachment_ref : VK_NULL_HANDLE;
...
{
	assert(rp);
	ASSERT_LOG_R1(config.depth_mode == PIGEON_VULKAN_RENDER_PASS_DEPTH_KEEP
	...
	get_attachments(config, attachments, &attachment_count, colour_attachment_refs, &depth_attachment_ref);

	VkSubpassDescription subpass = { 0 };
	VkSubpassDependency subpass_dependencies[1] = { { 0 } };
	unsigned int subpass_dependency_count;

	get_subpass(config, &subpass, subpass_dependencies, &subpass_dependency_count, colour_attachment_refs,
	...
	ASSERT_LOG_R1(vkCreateRenderPass(vkdev, &render_pass_create, NULL, &rp->vk_renderpass) == VK_SUCCESS,
