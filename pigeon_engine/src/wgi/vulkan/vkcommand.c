	...
	unsigned int buffer_count, bool primary, bool use_transfer_queue, bool one_shot)
{
	...
	if(vkCreateCommandPool(vkdev, &pool_create, NULL, &command_pool->vk_command_pool) != VK_SUCCESS) {
		...
		if(buffer_count > 1) free(command_pool->vk_command_buffers);
		return 1;
	}
	...
	if (vkAllocateCommandBuffers(vkdev, &alloc, 
			buffer_count > 1 ? command_pool->vk_command_buffers : &command_pool->vk_command_buffer) != VK_SUCCESS) {
...
{
	return command_pool->buffer_count == 1 ? 
	...
	PigeonVulkanRenderPass * render_pass, PigeonVulkanFramebuffer * framebuffer)
{
	assert(command_pool && command_pool->vk_command_pool && command_pool->vk_command_buffer);
	assert(buffer_index < command_pool->buffer_count);
	if(render_pass) assert(framebuffer);
...
void pigeon_vulkan_transition_image_to_transfer_dst(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index,
...
void pigeon_vulkan_set_viewport_size(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index, 
	...
	PigeonVulkanCommandPool* secondary_command_pool, unsigned int secondary_buffer_index)
{
	assert(command_pool && command_pool->vk_command_pool && command_pool->vk_command_buffer);
	assert(buffer_index < command_pool->buffer_count);
	assert(secondary_command_pool && secondary_command_pool->vk_command_pool && secondary_command_pool->vk_command_buffer);
...
void pigeon_vulkan_wait_for_depth_write(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index, 
	PigeonVulkanImage* image)
{
	assert(command_pool && command_pool->vk_command_pool && command_pool->vk_command_buffer);
	assert(buffer_index < command_pool->buffer_count);
	...
	PigeonVulkanImage* src, PigeonVulkanImage* dst, bool bilinear_filter)
{
	...
	VkImageMemoryBarrier image_memory_barriers[2] = {{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER},
	...
	image_memory_barriers[0].srcQueueFamilyIndex = 
	...
	image_memory_barriers[1].srcQueueFamilyIndex = 
	...
	vkCmdBlitImage(get_cmd_buf(command_pool, buffer_index), 
		src->vk_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
		dst->vk_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
		1, &region, bilinear_filter ? VK_FILTER_LINEAR : VK_FILTER_NEAREST
	);
}

	 
void pigeon_vulkan_transition_transfer_src_to_shader_read(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index,
	PigeonVulkanImage* image)
{
	assert(command_pool && command_pool->vk_command_pool && command_pool->vk_command_buffer);
...
void pigeon_vulkan_transition_transfer_dst_to_shader_read(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index,
	PigeonVulkanImage* image)
{
	assert(command_pool && command_pool->vk_command_pool && command_pool->vk_command_buffer);
	assert(image && image->vk_image);
...
void pigeon_vulkan_transfer_buffer_to_image(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index,
	...
	PigeonVulkanImage* image, unsigned int array_layer, unsigned int image_x, unsigned int image_y,
...
void pigeon_vulkan_bind_pipeline(PigeonVulkanCommandPool* command_pool, unsigned int buffer_index,
	...
	PigeonVulkanBuffer* buffer, unsigned int attribute_count, uint64_t* offsets)
{
	...
	vkCmdBindVertexBuffers(get_cmd_buf(command_pool, buffer_index),
		0, attribute_count, buffers, offsets);
}
	...
	PigeonVulkanBuffer* buffer, uint64_t offset, bool big_indices)
{
	...
	vkCmdBindIndexBuffer(get_cmd_buf(command_pool, buffer_index),
		buffer->vk_buffer, offset, big_indices ? VK_INDEX_TYPE_UINT32 : VK_INDEX_TYPE_UINT16);
}
