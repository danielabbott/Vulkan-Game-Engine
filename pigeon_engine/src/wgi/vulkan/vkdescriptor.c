...
int pigeon_vulkan_create_descriptor_layout(
	PigeonVulkanDescriptorLayout* layout, unsigned int bindings_count, PigeonVulkanDescriptorBinding* bindings)
{
...
static int create_pool(PigeonVulkanDescriptorPool* pool, unsigned int set_count, PigeonVulkanDescriptorLayout* layouts)
{
	...
	ASSERT_LOG_R1(vkCreateDescriptorPool(vkdev, &poolInfo, NULL, &pool->vk_descriptor_pool) == VK_SUCCESS,
		"vkCreateDescriptorPool error");

	return 0;
}

int pigeon_vulkan_create_descriptor_pool(
	PigeonVulkanDescriptorPool* pool, unsigned int set_count, PigeonVulkanDescriptorLayout* layouts)
{
	...
	ASSERT_LOG_R1(
		vkAllocateDescriptorSets(vkdev, &allocInfo, set_count > 1 ? pool->vk_descriptor_sets : &pool->vk_descriptor_set)
			== VK_SUCCESS,
...
static void do_update(PigeonVulkanDescriptorPool* pool, unsigned int set, unsigned int binding,
	unsigned int array_index, VkDescriptorBufferInfo* buffer_info, VkDescriptorImageInfo* image_info,
	bool buffer_is_ssbo)
{
...
void pigeon_vulkan_set_descriptor_uniform_buffer(PigeonVulkanDescriptorPool* pool, unsigned int set,
	unsigned int binding, unsigned int array_index, PigeonVulkanBuffer* buffer)
{
	pigeon_vulkan_set_descriptor_uniform_buffer2(pool, set, binding, array_index, buffer, 0, buffer->size);
}

void pigeon_vulkan_set_descriptor_uniform_buffer2(PigeonVulkanDescriptorPool* pool, unsigned int set,
	unsigned int binding, unsigned int array_index, PigeonVulkanBuffer* buffer, uint64_t offset, uint64_t range)
{
...
void pigeon_vulkan_set_descriptor_ssbo(PigeonVulkanDescriptorPool* pool, unsigned int set, unsigned int binding,
	unsigned int array_index, PigeonVulkanBuffer* buffer)
{
	pigeon_vulkan_set_descriptor_ssbo2(pool, set, binding, array_index, buffer, 0, buffer->size);
}

void pigeon_vulkan_set_descriptor_ssbo2(PigeonVulkanDescriptorPool* pool, unsigned int set, unsigned int binding,
	unsigned int array_index, PigeonVulkanBuffer* buffer, uint64_t offset, uint64_t range)
{
	assert(pool && set < pool->number_of_sets);
	assert(buffer && buffer->vk_buffer);
...
void pigeon_vulkan_set_descriptor_texture(PigeonVulkanDescriptorPool* pool, unsigned int set, unsigned int binding,
	unsigned int array_index, PigeonVulkanImageView* image_view, PigeonVulkanSampler* sampler)
{
	assert(pool && set < pool->number_of_sets);
	assert(image_view && image_view->vk_image_view && sampler && sampler->vk_sampler);

	VkDescriptorImageInfo image_info = { 0 };
	image_info.imageLayout = pigeon_wgi_image_format_is_depth(image_view->format)
...
void pigeon_vulkan_destroy_descriptor_pool(PigeonVulkanDescriptorPool* pool)
{
	...
	if (pool && pool->vk_descriptor_pool) {
		...
		if (pool->number_of_sets > 1)
			free2(pool->vk_descriptor_sets);
	}
}
