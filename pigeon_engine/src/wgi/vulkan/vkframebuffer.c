...
PIGEON_ERR_RET pigeon_vulkan_create_framebuffer2(PigeonVulkanFramebuffer* framebuffer, PigeonVulkanImageView* depth,
	PigeonVulkanImageView* colour, PigeonVulkanImageView* colour2, PigeonVulkanRenderPass* render_pass)
{
	...
	if (colour2) {
		...
		if (colour) {
			assert(framebuffer_create.width == colour2->width && framebuffer_create.height == colour2->height
	...
	if (depth) {
		...
		if (colour) {
			assert(framebuffer_create.width == depth->width && framebuffer_create.height == depth->height
...
PIGEON_ERR_RET pigeon_vulkan_create_framebuffer(PigeonVulkanFramebuffer* framebuffer, PigeonVulkanImageView* depth,
	PigeonVulkanImageView* colour, PigeonVulkanRenderPass* render_pass)
{
	ASSERT_R1(framebuffer && render_pass);
	ASSERT_R1(depth || colour);
	...
	if (depth) {
		...
		if (colour) {
			ASSERT_R1(framebuffer_create.width == depth->width && framebuffer_create.height == depth->height
				&& framebuffer_create.layers == depth->layers);
		}
