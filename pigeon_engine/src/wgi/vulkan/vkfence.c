...
PIGEON_ERR_RET pigeon_vulkan_create_fence(PigeonVulkanFence* fence, bool start_triggered)
{
	...
	ASSERT_LOG_R1(vkCreateFence(vkdev, &fence_create, NULL, &fence->vk_fence) == VK_SUCCESS,
		"vkCreateFence error");
	return 0;
}
...
{
	ASSERT_LOG_R1(vkWaitForFences(vkdev, 1, &fence->vk_fence, VK_TRUE, 3000000000) == VK_SUCCESS, "vkWaitForFences error");
	return 0;
}
