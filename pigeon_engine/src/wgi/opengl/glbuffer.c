...
PIGEON_ERR_RET pigeon_opengl_create_buffer(PigeonOpenGLBuffer* buffer, uint64_t size, PigeonOpenGLBufferType type)
{
	...
	glBufferData(t, (int64_t)size, NULL,
		(type == PIGEON_OPENGL_BUFFER_TYPE_UNIFORM || type == PIGEON_OPENGL_BUFFER_TYPE_TEXTURE) ? GL_STREAM_DRAW
																								 : GL_STATIC_DRAW);
...
void* pigeon_opengl_map_buffer(PigeonOpenGLBuffer* buffer)
{
	...
	buffer->mapping
		= glMapBufferRange(get_target(buffer->type), 0, buffer->size, GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
...
void pigeon_opengl_bind_uniform_buffer2(
	PigeonOpenGLBuffer* buffer, unsigned int bind_point, unsigned int offset, unsigned int size)
{
	assert(buffer && buffer->id && buffer->type == PIGEON_OPENGL_BUFFER_TYPE_UNIFORM);
	assert(offset + size <= buffer->size);

	if (!size)
		size = (unsigned int)buffer->size - offset;
	assert(size);
...
static void get_vertex_attribute_format(
	PigeonWGIVertexAttributeType type, unsigned int* comps, GLenum* gl_type, bool* norm, bool* gl_int)
{
	switch (type) {
...
PIGEON_ERR_RET pigeon_opengl_create_vao(PigeonOpenGLVAO* vao, unsigned int vertex_attribute_count,
	PigeonWGIVertexAttributeType vertex_attributes[], uint64_t attribute_start_offsets[], void** vertex_data,
	uint64_t vertex_data_size, bool big_indices, unsigned int index_count, void** index_data, uint64_t index_data_size)
{
...
#define CLEANUP() pigeon_opengl_destroy_vao(vao);
	...
	ASSERT_R1(!pigeon_opengl_create_buffer(&vao->vertex_buffer, vertex_data_size, PIGEON_OPENGL_BUFFER_TYPE_VERTEX));

	*vertex_data = pigeon_opengl_map_buffer(&vao->vertex_buffer);
	ASSERT_R1(*vertex_data);

	ASSERT_R1(!pigeon_opengl_create_buffer(&vao->index_buffer, index_data_size, PIGEON_OPENGL_BUFFER_TYPE_INDEX));
	...
	*index_data = pigeon_opengl_map_buffer(&vao->index_buffer);
	...
	for (unsigned int i = 0; i < vertex_attribute_count; i++) {
		...
		if (gl_int)
			glVertexAttribIPointer(i, comps, gl_type, 0, (void*)attribute_start_offsets[i]);
		else
			glVertexAttribPointer(i, comps, gl_type, norm, 0, (void*)attribute_start_offsets[i]);
