...
static PIGEON_ERR_RET include_file(char ** dst_, unsigned int* dst_sz_, unsigned * dst_i_, 
    const char * directory_path, char * file_name, bool first_file)
{
    char * dst = *dst_;
    ...
    unsigned int j = 0;
    while(file_name[j] && file_name[j] != '"') { j++; }
...
PIGEON_ERR_RET pigeon_opengl_load_shader(PigeonOpenGLShader* shader,
    const char* file_path, const char * directory_path, const char * prefix, PigeonOpenGLShaderType type)
{
    ...
    if(prefix) memcpy(&dst[18], prefix, prefix_len);
    dst[18+prefix_len] = '\n';
...
void pigeon_opengl_bind_shader_program(PigeonOpenGLShaderProgram* p)
{
    ...
    else bind_shader_id(0);
}

PIGEON_ERR_RET pigeon_opengl_create_shader_program(PigeonOpenGLShaderProgram* program,
	PigeonOpenGLShader* vs, PigeonOpenGLShader* fs,
    unsigned int vertex_attrib_count, const char* vertex_attrib_names[])
{
    ASSERT_R1(program && vs && vs->id);
    if(fs) ASSERT_R1(fs->id);
    ...
    if(fs) glAttachShader(program->id, fs->id);
...
void pigeon_opengl_set_uniform_buffer_binding(PigeonOpenGLShaderProgram* program, 
	const char * name, unsigned int bind_point)
{
...
void pigeon_opengl_set_shader_texture_binding_index(PigeonOpenGLShaderProgram* program,
	const char * name, unsigned int bind_point)
{
    assert(program && program->id && name);
...
void pigeon_opengl_set_uniform_vec4(PigeonOpenGLShaderProgram* program, int location, 
