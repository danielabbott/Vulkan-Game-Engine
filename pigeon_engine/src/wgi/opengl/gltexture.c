...
static void pigeon_get_opengl_image_format2(PigeonWGIImageFormat f, GLenum* format_format, GLenum* format_type)
{
	switch (f) {
	case PIGEON_WGI_IMAGE_FORMAT_NONE:
...
PIGEON_ERR_RET pigeon_opengl_create_texture(PigeonOpenGLTexture* texture, PigeonWGIImageFormat format, unsigned width,
	unsigned int height, unsigned int layers, unsigned int mip_levels)
{
...
void pigeon_opengl_set_texture_sampler(PigeonOpenGLTexture* texture, bool linear_filter, bool anisiotropic_filtering,
	...
	if (texture->mip_levels > 1) {
		if (linear_filter) {
			glTexParameteri(target, GL_TEXTURE_MIN_FILTER,
				multi_mip_map_sample ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR_MIPMAP_NEAREST);
		} else {
...
PIGEON_ERR_RET pigeon_opengl_upload_texture_mip(
	PigeonOpenGLTexture* texture, unsigned int mip, unsigned int layer, void* data, unsigned int data_size)
{
...
PIGEON_ERR_RET pigeon_opengl_create_buffer_texture(
	PigeonOpenGLBufferTexture* texture, unsigned int elements, unsigned int components_per_element)
{
	assert(texture && elements && components_per_element && components_per_element <= 4);

	ASSERT_R1(!pigeon_opengl_create_buffer(
		&texture->buffer, elements * components_per_element * 4, PIGEON_OPENGL_BUFFER_TYPE_TEXTURE));
...
bool pigeon_opengl_bc5_optimal_available(void) { return GLAD_GL_ARB_texture_compression_rgtc; }

bool pigeon_opengl_bc7_optimal_available(void) { return GLAD_GL_ARB_texture_compression_bptc; }

bool pigeon_opengl_etc1_optimal_available(void) { return GLAD_GL_ARB_ES3_compatibility; }

bool pigeon_opengl_etc2_optimal_available(void) { return GLAD_GL_ARB_ES3_compatibility; }

bool pigeon_opengl_etc2_rgba_optimal_available(void) { return GLAD_GL_ARB_ES3_compatibility; }
