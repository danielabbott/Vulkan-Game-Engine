...
PIGEON_ERR_RET pigeon_wgi_allocate_empty_grids(
	PigeonWGIGridTextureGrid** grid_ptr, unsigned int* grids_count, unsigned int tiles)
{
...
PIGEON_ERR_RET pigeon_wgi_add_texture_to_grid(PigeonWGIGridTextureGrid** grids_, unsigned int* grids_count,
	void* texture, unsigned int tiles_width, unsigned int tiles_height, PigeonWGITextureGridPosition* position,
	bool allow_add_new_grids)
{
	...
	*grids_count = new_grids_count;
	...
	return pigeon_wgi_add_texture_to_grid(grids_, grids_count, texture, tiles_width, tiles_height, position, false);
}

PIGEON_ERR_RET pigeon_wgi_create_grid_texture(
	PigeonWGIGridTexture* grid_texture, uint32_t grids, PigeonWGIImageFormat format, bool mip_maps)
{
	...
	ASSERT_R1(
		pigeon_wgi_create_array_texture(&grid_texture->array_texture, 4096, 4096, grids, format, mip_maps ? 8 : 1));
	return 0;
}
...
uint32_t pigeon_wgi_get_grid_texture_tile_size(PigeonWGIImageFormat format, bool mip_maps)
{
	if (mip_maps)
		...
		return ((512 * 512) / 16) * pigeon_image_format_bytes_per_4x4_block(format);
}

void* pigeon_wgi_grid_texture_upload(
	PigeonWGIGridTexture* grid_texture, PigeonWGITextureGridPosition position, unsigned int w, unsigned int h)
{
	assert(w % 512 == 0);
	assert(h % 512 == 0);
	...
	grid_texture->array_texture.mapping_offset += position.grid_w * position.grid_h
		* pigeon_wgi_get_grid_texture_tile_size(
			grid_texture->array_texture.format, grid_texture->array_texture.mip_maps > 1);

	pigeon_vulkan_transfer_buffer_to_image(get_upload_cmd_pool(), 0, &grid_texture->array_texture.data->staging_buffer,
		buffer_offset, &grid_texture->array_texture.data->image, position.grid_i, position.grid_x * 512,
		position.grid_y * 512, w, h, grid_texture->array_texture.mip_maps ? 8 : 1);
