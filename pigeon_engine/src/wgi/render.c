...
PIGEON_ERR_RET pigeon_wgi_create_per_frame_objects()
{
    ...
    for(unsigned int i = 0; i < singleton_data.frame_objects_count; i++) {
        ...
        if(VULKAN) {
            ASSERT_R1(!pigeon_vulkan_create_command_pool(&objects->primary_command_pools[0], 1, true, false));
            ASSERT_R1(!pigeon_vulkan_create_command_pool(&objects->primary_command_pools[1], 1, true, false));
            ASSERT_R1(!pigeon_vulkan_create_command_pool(&objects->primary_command_pools[2], 1, true, false));
        }
        ...
        if(VULKAN) {
            ...
            for(unsigned int j = 0; j < 3; j++) {
                for(unsigned int k = 0; k < 2; k++) {
                    ASSERT_R1(!pigeon_vulkan_create_semaphore(&objects->semaphores[j][k]));
                }
            }
            ...
            if(pigeon_vulkan_general_queue_supports_timestamps())
                ASSERT_R1(!pigeon_vulkan_create_timer_query_pool(&objects->timer_query_pool, 2*PIGEON_WGI_TIMERS_COUNT));
            ...
            ASSERT_R1(!pigeon_opengl_create_timer_query_group(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMERS_COUNT));
        }
...
void pigeon_wgi_destroy_per_frame_objects()
{
    ...
    if(VULKAN) {
        for(unsigned int i = 0; i < singleton_data.frame_objects_count; i++) {
            ...
            for(unsigned int j = 0; j < 3; j++) {
                for(unsigned int k = 0; k < 2; k++) {
                    pigeon_vulkan_destroy_semaphore(&objects->semaphores[j][k]);
                }
            }
            ...
            pigeon_vulkan_destroy_command_pool(&objects->primary_command_pools[0]);
            pigeon_vulkan_destroy_command_pool(&objects->primary_command_pools[1]);
            pigeon_vulkan_destroy_command_pool(&objects->primary_command_pools[2]);
...
PIGEON_ERR_RET pigeon_wgi_next_frame_poll(double delayed_timer_values[PIGEON_WGI_TIMERS_COUNT], bool* ready)
{
    ...
    if(OPENGL) {
        ...
        if(!objects->first_frame_submitted) {        
            ...
            double completion_times[1+PIGEON_WGI_TIMERS_COUNT];
            pigeon_opengl_get_timer_query_results(&objects->gl.timer_queries, completion_times);

            for(unsigned int i = 0; i < PIGEON_WGI_TIMERS_COUNT; i++) {
                delayed_timer_values[i] = completion_times[i+1] - completion_times[i];
            }
        }
    ...
    if(!ready || *ready == true) {
        double absolute_times[2*PIGEON_WGI_TIMERS_COUNT];
    ...
    PigeonWGIBoneMatrix ** bone_matrices)
{
    ...
    if(VULKAN) {
        ...
        *bone_matrices = (PigeonWGIBoneMatrix *) (void *) dst;
        ...
        pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_UPLOAD); // not used for opengl
    }    
...
static PIGEON_ERR_RET pigeon_wgi_start_command_buffer_gl(PigeonWGICommandBuffer * command_buffer)
{
    ...
    if(command_buffer == &objects->depth_command_buffer) {
        ...
        pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_SHADOW_MAPS);
            ...
            command_buffer < &objects->shadow_command_buffers[0]+4)
    {
        ...
        pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_DEPTH_PREPASS);
        ...
        pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_SSAO);
        ...
        pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_SSAO_BLUR);  
    }
    else {
        ASSERT_R1(false);
    }
...
static PipelineOrProgram get_pipeline(PigeonWGICommandBuffer* command_buffer, PigeonWGIPipeline* pipeline)
{
    if(VULKAN) {
        ...
        return command_buffer->depth_only ?
            pipeline->gl.program_depth : pipeline->gl.program;
    }
}
...
PIGEON_ERR_RET pigeon_wgi_end_command_buffer(PigeonWGICommandBuffer * command_buffer)
{
    ...
    if(OPENGL) {
        ...
        if(command_buffer == &objects->render_command_buffer) {
            pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_RENDER);
            pigeon_opengl_set_draw_state(&singleton_data.gl.full_screen_tri_cfg);
            ...
            pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_BLOOM_BLUR);
            ...
            pigeon_opengl_set_timer_query_value(&objects->gl.timer_queries, 1+PIGEON_WGI_TIMER_POST_PROCESS);            
        }
    }
    return 0;
}
...
PIGEON_ERR_RET pigeon_wgi_present_frame_rec_sub0(void)
{
    if(OPENGL) return 0;
    ...
    ASSERT_R1(objects->depth_command_buffer.has_been_recorded);

    PigeonVulkanCommandPool * p = &objects->primary_command_pools[0];
    ...
    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        ...
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_UPLOAD);
    }

    if(objects->upload_command_buffer.has_been_recorded) {
        use_secondary_command_buffer(p, 0,
        ...
        pigeon_vulkan_wait_for_vertex_data_transfer(p, 0);
    }

    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_UPLOAD);
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_SHADOW_MAPS);
    }
    ...
    // Shadows
    for(unsigned int i = 0; i < 4; i++) {
        PigeonWGIShadowParameters* sp = &singleton_data.shadow_parameters[i];
        if(!sp->resolution) continue;

        ASSERT_R1(sp->framebuffer_index >= 0);
        ASSERT_R1(objects->shadow_command_buffers[i].has_been_recorded);

        unsigned int fb_idx = (unsigned)sp->framebuffer_index;
        unsigned int w = singleton_data.shadow_images[fb_idx].image.width;
        unsigned int h = singleton_data.shadow_images[fb_idx].image.height;
        PigeonVulkanFramebuffer* fb = &singleton_data.shadow_framebuffers[fb_idx];
    
        use_secondary_command_buffer(p, 0,
            &objects->shadow_command_buffers[i], &singleton_data.rp_depth,
            fb, w, h);
        objects->shadow_command_buffers[i].has_been_recorded = false;
    }

    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_SHADOW_MAPS);
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_DEPTH_PREPASS);
    }
    ...
    objects->depth_command_buffer.has_been_recorded = false;
    
    pigeon_vulkan_wait_for_depth_write(p, 0, &singleton_data.depth_image.image);

    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_DEPTH_PREPASS);
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_SSAO);
    }
    ...
    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_SSAO);
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_SSAO_BLUR);
    }
    ...
    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_SSAO_BLUR);
    }


    ASSERT_R1(!pigeon_vulkan_submit3(p, 0, &objects->pre_render_done_fence, 
        first_frame ? NULL : &prev_objects->semaphores[1][1], NULL,
        &objects->semaphores[0][0], /*&objects->semaphores[0][1]*/ NULL));

    return 0;
}

// TODO either have multiple secondary command pools or record direct to primary
PIGEON_ERR_RET pigeon_wgi_present_frame_rec1(void)
{
    if(OPENGL) return 0;
    ...
    ASSERT_R1(objects->render_command_buffer.has_been_recorded);

    PigeonVulkanCommandPool * p = &objects->primary_command_pools[1];

    ASSERT_R1(!pigeon_vulkan_reset_command_pool(p));
    ASSERT_R1 (!pigeon_vulkan_start_submission(p, 0));

    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_RENDER);
    }

    use_secondary_command_buffer(p, 0,
        &objects->render_command_buffer, &singleton_data.rp_render,
        &singleton_data.render_framebuffer, sc_info.width, sc_info.height);
    objects->render_command_buffer.has_been_recorded = false;
    

    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_RENDER);
    }
    return 0;
}

PIGEON_ERR_RET pigeon_wgi_present_frame_rec2(void)
{
    if(OPENGL) return 0;
    ...
    PigeonVulkanCommandPool * p = &objects->primary_command_pools[2];
    ASSERT_R1(!pigeon_vulkan_reset_command_pool(p));
    ASSERT_R1(!pigeon_vulkan_start_submission(p, 0));
    ...
    if(singleton_data.render_cfg.bloom) {
        ...
        if(pigeon_vulkan_general_queue_supports_timestamps()) {
            pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_BLOOM_BLUR);
        }
    ...
    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_BLOOM_BLUR);
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 2*PIGEON_WGI_TIMER_POST_PROCESS);
    }
    ...
    pigeon_vulkan_set_viewport_size(p, 0, sc_info.width, sc_info.height);
    pigeon_vulkan_start_render_pass(p, 0, &singleton_data.rp_post, 
    ...
    if(pigeon_vulkan_general_queue_supports_timestamps()) {
        pigeon_vulkan_set_timer(p, 0, &objects->timer_query_pool, 1+2*PIGEON_WGI_TIMER_POST_PROCESS);
    }
...
PIGEON_ERR_RET pigeon_wgi_present_frame_sub1(void)
{
    PerFrameData * objects = &singleton_data.per_frame_objects[singleton_data.current_frame_index_mod];
    ...
    ASSERT_R1(!pigeon_vulkan_submit3(&objects->primary_command_pools[1], 0, NULL, 
        &objects->semaphores[0][0], first_frame ? NULL : &prev_objects->semaphores[2][1],
        &objects->semaphores[1][0], &objects->semaphores[1][1]));

    
    ASSERT_R1(!pigeon_vulkan_submit3(&objects->primary_command_pools[2], 0, NULL, 
        &objects->semaphores[1][0], &objects->swapchain_acquire_semaphore,
        &objects->semaphores[2][0], &objects->semaphores[2][1]));
    ...
    int swapchain_err = pigeon_vulkan_swapchain_present(&objects->semaphores[2][0]);
    if(swapchain_err == 2) {
        ASSERT_R1(!recreate_swapchain_loop());
        swapchain_err = 0;
    }
