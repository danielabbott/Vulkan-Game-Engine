...
typedef enum {
	...
	PIGEON_OPENGL_SHADER_TYPE_FRAGMENT
} PigeonOpenGLShaderType;

PIGEON_ERR_RET pigeon_opengl_load_shader(PigeonOpenGLShader*, const char* file_path, 
	const char * directory_path, const char * prefix, PigeonOpenGLShaderType);
...
PIGEON_ERR_RET pigeon_opengl_create_shader_program(PigeonOpenGLShaderProgram*,
...
void pigeon_opengl_set_uniform_buffer_binding(PigeonOpenGLShaderProgram*, 
	const char * name, unsigned int bind_point);

void pigeon_opengl_set_shader_texture_binding_index(PigeonOpenGLShaderProgram*,
	const char * name, unsigned int bind_point);
