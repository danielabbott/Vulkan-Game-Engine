...
PIGEON_ERR_RET pigeon_vulkan_create_image(PigeonVulkanImage*, PigeonWGIImageFormat, unsigned width, unsigned int height,
	unsigned int layers, unsigned int mip_levels,
	...
	bool to_be_transfer_src, bool to_be_transfer_dst, PigeonVulkanMemoryRequirements*);

PIGEON_ERR_RET pigeon_vulkan_image_bind_memory(PigeonVulkanImage*, PigeonVulkanMemoryAllocation*, unsigned int offset);
...
PIGEON_ERR_RET pigeon_vulkan_create_image_view2(
	PigeonVulkanImageView*, PigeonVulkanImage*, bool array_texture, unsigned int layer);
