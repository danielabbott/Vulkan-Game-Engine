...
PIGEON_ERR_RET pigeon_wgi_allocate_empty_grids(
	PigeonWGIGridTextureGrid**, unsigned int* grids_count, unsigned int tiles);
...
PIGEON_ERR_RET pigeon_wgi_add_texture_to_grid(PigeonWGIGridTextureGrid**, unsigned int* grids_count,
	...
	unsigned int tiles_width, unsigned int tiles_height, PigeonWGITextureGridPosition*, bool allow_add_new_grids);
...
PIGEON_ERR_RET pigeon_wgi_create_array_texture(
	PigeonWGIArrayTexture*, uint32_t width, uint32_t height, uint32_t layers, PigeonWGIImageFormat,
...
void* pigeon_wgi_array_texture_upload1(PigeonWGIArrayTexture*, unsigned int layer);

// Call this once for each layer, this does the memcpy
PIGEON_ERR_RET pigeon_wgi_array_texture_upload2(PigeonWGIArrayTexture*, unsigned int layer, void*);
...
PIGEON_ERR_RET pigeon_wgi_create_grid_texture(
	PigeonWGIGridTexture*, uint32_t grids, PigeonWGIImageFormat, bool mip_maps);
...
void* pigeon_wgi_grid_texture_upload(
	PigeonWGIGridTexture*, PigeonWGITextureGridPosition, unsigned int w, unsigned int h);
