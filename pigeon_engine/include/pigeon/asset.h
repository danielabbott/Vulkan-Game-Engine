...
typedef enum {
    PIGEON_ASSET_COMPRESSION_TYPE_NONE,
    PIGEON_ASSET_COMPRESSION_TYPE_ZSTD,
} PigeonAssetCompressionType;

typedef struct PigeonAssetCompression {
    PigeonAssetCompressionType type;
...
typedef struct PigeonAsset {
    ...
    PigeonAssetCompression compression[PIGEON_ASSET_MAX_SUBRESOURCES];
...
// buffer must be >= asset->compression[i].decompressed_data_length
