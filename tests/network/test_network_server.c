...
static void new_connection(PigeonIOHub* io_hub, PigeonIOHubConnectionHandle h,
...
;
    ...
    if(pigeon_io_hub_tcp_send(io_hub, h, s, (unsigned)strlen(s), 0) ||
		pigeon_io_hub_tcp_shutdown_read(io_hub, h))
	{
		assert(false);
        exit(1);
    }
...
PIGEON_ERR_RET pigeon_test_server(void)
{
	...
	ASSERT_R1(!pigeon_network_create_server_socket_blocking(&tcp_socket, 
		25565, PIGEON_NETWORK_PROTOCOL_TCP));

	ASSERT_R1(!pigeon_network_create_server_socket_blocking(&udp_socket, 
		25565, PIGEON_NETWORK_PROTOCOL_UDP));
